...
export const tagBounty = async (request: TagRequest): Promise<void> => {
    ...
    const getDbResult: {
        dbBountyResult: BountyCollection;
        bountyChannel: string;
    } = await getDbHandler(request);

    await writeDbHandler(request);

    const bountyCard = await BountyUtils.canonicalCard(
        getDbResult.dbBountyResult._id,
        request.activity
    );
...
const writeDbHandler = async (request: TagRequest): Promise<void> => {
    ...
    const writeResult: UpdateWriteOpResult = await bountyCollection.updateOne(
...
const getDbHandler = async (
    request: TagRequest
): Promise<{ dbBountyResult: BountyCollection; bountyChannel: string }> => {
    const db: Db = await MongoDbUtils.connect('bountyboard');
    const bountyCollection = db.collection('bounties');
    ...
    const dbCustomerResult: CustomerCollection =
